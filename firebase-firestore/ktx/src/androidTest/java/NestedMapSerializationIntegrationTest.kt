...
import com.google.firebase.firestore.ktx.serialization.getData
import com.google.firebase.firestore.ktx.serialization.setData
import com.google.firebase.firestore.ktx.testutil.IntegrationTestUtil.Companion.testDocument
import com.google.firebase.firestore.ktx.testutil.IntegrationTestUtil.Companion.waitFor
...
class NestedMapSerializationIntegrationTest {
    ...
    fun testSerializationSetMethodWorksForNestedObject() {
        ...
        for (project in projectList) {
            docRefKotlin.setData<ObjectInsideOfObject>(project)
            docRefPOJO.set(project)
            val expected = waitFor(docRefPOJO.get()).data
            val actual = waitFor(docRefKotlin.get()).data
            assertEquals(expected, actual)
    ...
    fun testSerializationSetMethodWorksForList() {
        ...
        for (project in projectList) {
            docRefKotlin.setData<ListOfObjectsInsideOfObject>(project)
            docRefPOJO.set(project)
            val expected = waitFor(docRefPOJO.get()).data
            val actual = waitFor(docRefKotlin.get()).data
            assertEquals(expected, actual)
    ...
    fun testSerializationGetMethodWorksForNestedObject() {
        ...
        for (project in projectList) {
            ...
            val actual = waitFor(docRefKotlin.get()).getData<ObjectInsideOfObject>()
            assertEquals(expected, actual)
        }
    }
    ...
    fun testSerializationGetMethodWorksForList() {
        ...
        for (project in projectList) {
            ...
            val actual = waitFor(docRefKotlin.get()).getData<ListOfObjectsInsideOfObject>()
            assertEquals(expected, actual)
        }
    }
    ...
    fun testSerializationWorksWithDocumentIdAnnotation() {
        ...
        for (project in kProjectList) {
            docRefKotlin.setData<KProjectWithDocId>(project)
            val expected = docRefKotlin.id
            val actual = waitFor(docRefKotlin.get()).getData<KProjectWithDocId>()?.owner
            assertEquals(expected, actual)
        }
    }
    ...
    fun testSerializationWorksWithThrowOnExtraProperties() {
        ...
        for (project in kProjectList) {
            docRefKotlin.setData<KProjectWithPrimitiveExtraProperties>(project)
            assertFailsWith<IllegalArgumentException>(
                message = "Can not match",
                block = {
                    waitFor(docRefKotlin.get()).getData<Project>()
                }
            )
        }
    }
    ...
    fun testSerializationWorksWithThrowOnExtraPropertiesOnCustomField() {
        ...
        for (project in kProjectList) {
            docRefKotlin.setData<KProjectWithCustomObjectExtraProperties>(project)
            assertFailsWith<IllegalArgumentException>(
                message = "Can not match",
                block = {
                    waitFor(docRefKotlin.get()).getData<Project>()
    ...
    fun testSavingReadingEnum(){
        ...
        docRefKotlin.setData<Payment>(expected)
        val actual = waitFor(docRefKotlin.get()).getData<Payment>()
        assertEquals(expected, actual)
    }
}
